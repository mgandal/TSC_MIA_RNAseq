picard.align <- read.table("./RNAseqAlign.txt",skip=1)
...
## Transcript level QC
picard.rnaseq.qc <- read.table("./RNAseqQC.txt",skip=1)
colnames(picard.rnaseq.qc) <- c("FileName","PF_BASES","PF_ALIGNED_BASES","CODING_BASES","UTR_BASES","INTRONIC_BASES","INTERGENIC_BASES","IGNORED_READS","CORRECT_STRAND_READS","INCORRECT_STRAND_READS",
                                ...
                                tx.coverage <- read.table("./TranscriptCoverage.txt") ## Need to go into this file and manually remove blank lines: HSB155.V1C_SE.bam was blank
                                ...
                                colnames(picard.rnaseq.gc) <- as.character(tmp[1,seq(2,601,by=6)])
                                rownames(picard.rnaseq.gc) <- tmp[,1]
                                picard.rnaseq.gc <- tmp[,seq(6,601,by=6)]
                                ...
                                PQCdat <- cbind(picard.align[,c(2,6,9)],picard.rnaseq.qc[,c(3:7,18:21)],
                                gc.summary[,c(4,5)])
