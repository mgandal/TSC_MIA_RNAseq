...
plotDendroAndColors(tree,colors = cbind(as.factor(datMeta$Genotype), as.factor(datMeta$Region), as.factor(datMeta$Treatment), as.factor(datMeta$Hemisphere), seqdepth_col), groupLabels = c("Genotype", "Region", "Treatment", "Hemisphere","SeqBatch", "Seq Depth"))
...
for (reg_ind in 1:length(regions)){
  ...
  for (i in 1:length(list_contrasts)){
    ...
    DESeq2::plotMA(curr_res,ylim=c(-2,2))
    title(paste(curr_reg, contrast))  
    
    c= rgb(t(col2rgb(as.numeric(1))),alpha=100,maxColorValue = 255)
    plot(curr_res$log2FoldChange, -log10(curr_res$padj), xlab="Log2 Fold Change", ylab="log10(P.adj)",pch=19,col=c,cex=.5) #, xlim = c(-2,2), ylim=c(0,40))
